...
import type { CalendarEvent } from "../components/ShowCalendar";
...
type NormalizedEvent = Event & { timestamp: number | null };
...
export default async function ShowsPage() {
  ...
  try {
    ...
    const normalized: NormalizedEvent[] =
      (data.shows?.nodes as ShowNode[] | undefined)
        ?.map<NormalizedEvent>((e) => {
    ...
    const datedEvents = normalized.filter((ev) => typeof ev.timestamp === "number") as (NormalizedEvent & {
      timestamp: number;
    })[];

    upcomingEvents = datedEvents
      .filter((ev) => ev.timestamp >= nowTime)
  ...
  const renderEventGrid = (events: Event[], emptyText: string) => (
    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      {events.length === 0 ? (
        <p className="col-span-full text-center text-3xl text-gray-500">{emptyText}</p>
      ) : (
        events.map((ev, i) => (
          <Link
            key={i}
          ...
          >
            ...
            <div className="p-8">
              ...
              <h3 className="text-2xl font-black text-blood-300 mt-2">{ev.venue}</h3>
  ...
  return (
    <section className="bg-black text-white">
      <div className="max-w-7xl mx-auto px-0">
        ...
        {renderEventGrid(upcomingEvents, "No upcoming shows yet. Chaos is brewing...")}

        <CalendarToggle events={upcomingEvents} />

        <div className="mt-20">
          <PageBanner title="PAST SHOWS" />
          {renderEventGrid(pastEvents, "No past shows logged yet.")}
        </div>
      </div>
    </section>
  );
